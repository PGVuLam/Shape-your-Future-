    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center space-x-2">
              ...
              {recScore && (
                <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-500/30 text-emerald-200 border border-emerald-400/30">
                  {recScore.overallScore}% {language === 'vi' ? 'Độ tương thích' : 'Profile Fit'}
                </span>
              )}
            </div>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 px-6 overflow-x-auto no-scrollbar">
          ...
          >
            {language === 'vi' ? 'Tổng quan & Nhiệm vụ' : 'Overview & Tasks'}
          </button>
          <button
          ...
          >
            {language === 'vi' ? `Kiểm toán Kỹ năng (${skillGap.overallReadiness}%)` : `Skill Gap (${skillGap.overallReadiness}% Readiness)`}
          </button>
          <button
          ...
          >
            {language === 'vi' ? 'Hướng Đào tạo & Ngành học' : 'Educational Pathways & Majors'}
          </button>
          <button
          ...
          >
            {language === 'vi' ? 'Thử nghiệm "Try-Before-Decide"' : '"Try Before You Decide" Experiments'}
          </button>
        </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {career.tasks.map((task, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl flex items-start space-x-2"
                    >
              ...
              <div className="grid md:grid-cols-2 gap-6 bg-slate-50/60 p-4 rounded-xl border border-slate-200">
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-600 mb-2">
                    {language === 'vi' ? 'Đối chiếu Tương quan Holland RIASEC' : 'Holland RIASEC Dimension Alignment'}
                  </h4>
                ...
                <div className="space-y-4 flex flex-col justify-center">
                  ...
                  <div className="p-3 bg-white rounded-lg border border-slate-200">
                    <span className="text-xs font-semibold text-slate-500">
                      {language === 'vi' ? 'Phong cách làm việc & Áp lực' : 'Work Style & Stress Profile'}
                    </span>
                  ...
                  <div className="p-3 bg-amber-50/80 border border-amber-200/80 rounded-lg text-xs space-y-1">
                    ...
                    <p className="text-amber-800 text-[11px] leading-relaxed">
                      {language === 'vi' ? t.salaryDisclaimer : career.salaryInfo.disclaimer}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-bold text-indigo-900">
                    {language === 'vi' ? 'Chỉ số Sẵn sàng Năng lực' : 'Calculated Competency Readiness'}
                  </h4>
            ...
            <div className="space-y-6">
                    ...
                    <div key={idx} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                      ...
                      <div className="pt-2 border-t border-slate-200/80 text-[11px] text-slate-600">
                        <span className="font-semibold text-slate-800">
                          {language === 'vi' ? 'Đặc điểm & Đánh đổi:' : 'Trade-offs & Considerations:'}
                        </span>{' '}
                        {path.tradeoffs}
                      </div>
                    </div>
              ...
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                <h5 className="text-xs font-bold text-slate-700 mb-2">
                  {language === 'vi' ? 'Các ngành học Đại học / Cao đẳng liên quan' : 'Relevant University / College Majors'}
                </h5>
            ...
            <div className="space-y-6">
                  ...
                  <div key={idx} className="p-4 bg-sky-50/70 border border-sky-200 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-semibold px-2 py-0.5 rounded bg-sky-100 text-sky-800">
                        {language === 'vi' ? 'Thời gian:' : 'Time:'} {exp.duration}
                      </span>
                    </div>
                    ...
                    <div className="pt-2 border-t border-sky-200/80 text-[11px] text-sky-800">
                      <span className="font-bold">{language === 'vi' ? 'Câu hỏi Tự đánh giá sau trải nghiệm:' : 'Evaluation Question:'}</span> {exp.evaluationQuestion}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <h5 className="text-xs font-bold text-slate-800">
                  {language === 'vi' ? 'Dự án Khởi đầu Gợi ý' : 'Recommended Starter Projects'}
                </h5>
